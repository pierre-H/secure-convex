...
describe("valibotToConvex", () => {
    ...
    describe("literal types", () => {
        ...
        it("should throw error for symbol literal", () => {
            const schema = { type: "literal", literal: Symbol("test") };
            expect(() => valibotToConvex(schema)).toThrow("Convex doesn't support symbols as literals");
        });
    });
    describe("instance types", () => {
        ...
        it("should throw error for unsupported instance", () => {
            const schema = { type: "instance", expects: "Date" };
            expect(() => valibotToConvex(schema)).toThrow("Unsupported instance type: Date");
        });
    });
    describe("convex id types", () => {
        ...
        it("should convert optional convexId", () => {
            ...
            expect(result.isOptional).toBe("optional");
        });
        it("should convert convexId in object fields", () => {
            ...
            expect(result.isOptional).toBe("required");
            // Check id field
            expect(result.fields.id.kind).toBe("id");
            expect(result.fields.id.tableName).toBe("users");
            expect(result.fields.id.isOptional).toBe("required");
            // Check name field
            expect(result.fields.name.kind).toBe("string");
            expect(result.fields.name.isOptional).toBe("required");
            // Check optional parentId field
            expect(result.fields.parentId.kind).toBe("id");
            expect(result.fields.parentId.tableName).toBe("users");
            expect(result.fields.parentId.isOptional).toBe("optional");
            // Check categoryId field
            expect(result.fields.categoryId.kind).toBe("id");
            expect(result.fields.categoryId.tableName).toBe("categories");
            expect(result.fields.categoryId.isOptional).toBe("required");
        });
        it("should convert array of convexIds", () => {
            ...
            expect(result.isOptional).toBe("required");
            expect(result.element.kind).toBe("id");
            expect(result.element.tableName).toBe("tags");
            expect(result.element.isOptional).toBe("required");
        });
        it("should convert record with convexId values", () => {
            ...
            expect(result.key.isOptional).toBe("required");
            expect(result.value.kind).toBe("id");
            expect(result.value.tableName).toBe("items");
            expect(result.value.isOptional).toBe("required");
        });
        it("should convert union with convexIds", () => {
            ...
            expect(["users", "admins"]).toContain(result.tableName);
        });
        it("should convert mixed union with convexId and primitives", () => {
            ...
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("id");
            expect(result.members[1].tableName).toBe("references");
        });
    });
    describe("array types", () => {
        it("should convert array schema", () => {
            ...
            expect(result.kind).toBe("array");
            expect(result.isOptional).toBe("required");
            expect(result.element.kind).toBe("string");
            expect(result.element.isOptional).toBe("required");
        });
        it("should convert array of numbers", () => {
            ...
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("array");
            expect(result.element.kind).toBe("float64");
        });
        it("should convert array of objects", () => {
            const schema = valibot.array(valibot.object({
                id: valibot.number(),
                name: valibot.string(),
            }));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("array");
            expect(result.element.kind).toBe("object");
            expect(result.element.fields.id.kind).toBe("float64");
            expect(result.element.fields.name.kind).toBe("string");
        });
        it("should convert array of unions", () => {
            const schema = valibot.array(valibot.union([valibot.string(), valibot.number()]));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("array");
            expect(result.element.kind).toBe("union");
            expect(result.element.members).toHaveLength(2);
            expect(result.element.members[0].kind).toBe("string");
            expect(result.element.members[1].kind).toBe("float64");
        });
        it("should convert nested array schema", () => {
            ...
            expect(result.element.element.kind).toBe("float64");
        });
        it("should convert array of literals", () => {
            const schema = valibot.array(valibot.literal("status"));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("array");
            expect(result.element.kind).toBe("literal");
        });
    });
    describe("object types", () => {
        it("should convert object schema", () => {
            ...
            expect(result.fields.name.kind).toBe("string");
            expect(result.fields.age.kind).toBe("float64");
        });
        ...
        it("should convert object with mixed types", () => {
            ...
            expect(result.isOptional).toBe("required");
            expect(result.fields).toBeDefined();
            expect(result.fields.id.kind).toBe("int64");
            expect(result.fields.id.isOptional).toBe("required");
            expect(result.fields.name.kind).toBe("string");
            expect(result.fields.name.isOptional).toBe("required");
            expect(result.fields.active.kind).toBe("boolean");
            expect(result.fields.active.isOptional).toBe("required");
            expect(result.fields.score.kind).toBe("float64");
            expect(result.fields.score.isOptional).toBe("required");
            expect(result.fields.tags.kind).toBe("array");
            expect(result.fields.tags.isOptional).toBe("required");
        });
        it("should convert object with mixed required and optional fields", () => {
            ...
            expect(result.kind).toBe("object");
            expect(result.isOptional).toBe("required");
            expect(result.fields.id.kind).toBe("string");
            expect(result.fields.id.isOptional).toBe("required");
            expect(result.fields.name.kind).toBe("string");
            expect(result.fields.name.isOptional).toBe("required");
            expect(result.fields.email.kind).toBe("string");
            expect(result.fields.email.isOptional).toBe("optional");
            expect(result.fields.phone.kind).toBe("string");
            expect(result.fields.phone.isOptional).toBe("optional");
            expect(result.fields.address.kind).toBe("string");
            expect(result.fields.address.isOptional).toBe("required");
        });
        it("should convert object with optional fields", () => {
            ...
            expect(result.fields.email.kind).toBe("string");
            expect(result.fields.email.isOptional).toBe("optional");
            expect(result.fields.phone.kind).toBe("union");
            expect(result.fields.phone.isOptional).toBe("required");
        });
        it("should convert object with literal fields", () => {
            ...
            expect(result.kind).toBe("object");
            expect(result.fields).toBeDefined();
            expect(result.fields.type.kind).toBe("literal");
            expect(result.fields.type.value).toBe("user");
            expect(result.fields.status.kind).toBe("literal");
            expect(result.fields.status.value).toBe("active");
            expect(result.fields.version.kind).toBe("literal");
            expect(result.fields.version.value).toBe(1);
        });
        it("should convert nested object schema", () => {
            ...
            expect(result.kind).toBe("object");
            expect(result.fields).toBeDefined();
            expect(result.fields.user.kind).toBe("object");
            expect(result.fields.settings.kind).toBe("object");
            // Validate nested object fields
            expect(result.fields.user.fields.name.kind).toBe("string");
            expect(result.fields.user.fields.email.kind).toBe("string");
            expect(result.fields.settings.fields.theme.kind).toBe("string");
            expect(result.fields.settings.fields.notifications.kind).toBe("boolean");
        });
        it("should convert object with union fields", () => {
            ...
            expect(result.kind).toBe("object");
            expect(result.fields).toBeDefined();
            expect(result.fields.value.kind).toBe("union");
            expect(result.fields.value.members).toHaveLength(2);
            expect(result.fields.value.members[0].kind).toBe("string");
            expect(result.fields.value.members[1].kind).toBe("float64");
            expect(result.fields.type.kind).toBe("literal"); // deduplicated by uniqueUnions
        });
        it("should convert deeply nested object", () => {
            ...
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("object");
            expect(result.fields.level1.kind).toBe("object");
            expect(result.fields.level1.fields.level2.kind).toBe("object");
            expect(result.fields.level1.fields.level2.fields.level3.kind).toBe("object");
            expect(result.fields.level1.fields.level2.fields.level3.fields
                .value.kind).toBe("string");
        });
    });
    describe("record types", () => {
        it("should convert record schema", () => {
            ...
            expect(result.key.kind).toBe("string");
            expect(result.key.isOptional).toBe("required");
            expect(result.value.kind).toBe("float64");
            expect(result.value.isOptional).toBe("required");
        });
        ...
        it("should convert record with object values", () => {
            const schema = valibot.record(valibot.string(), valibot.object({
                name: valibot.string(),
            ...
            expect(result.value.fields.name.kind).toBe("string");
            expect(result.value.fields.count.kind).toBe("float64");
        });
        it("should convert record with union values", () => {
            const schema = valibot.record(valibot.string(), valibot.union([
                valibot.string(),
                valibot.number(),
                valibot.boolean(),
            ]));
            ...
            expect(result.kind).toBe("record");
            expect(result.key.kind).toBe("string");
            expect(result.value.kind).toBe("union");
            expect(result.value.members).toHaveLength(3);
            expect(result.value.members[0].kind).toBe("string");
            expect(result.value.members[1].kind).toBe("float64");
            expect(result.value.members[2].kind).toBe("boolean");
        });
        it("should convert record with array values", () => {
            const schema = valibot.record(valibot.string(), valibot.array(valibot.string()));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("record");
            expect(result.key.kind).toBe("string");
            expect(result.value.kind).toBe("array");
            expect(result.value.element.kind).toBe("string");
        });
    });
    describe("union types", () => {
        it("should convert union schema", () => {
            ...
            expect(result.members).toHaveLength(2);
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("float64");
        });
        ...
        it("should handle complex union with multiple types", () => {
            ...
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("float64");
            expect(result.members[2].kind).toBe("boolean");
        });
        ...
        it("should handle mixed union with primitives and literals", () => {
            ...
            expect(result.members).toHaveLength(3);
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("literal");
            expect(result.members[1].value).toBe(42);
            expect(result.members[2].kind).toBe("boolean");
        });
        ...
        it("should handle union with mixed literal types preserving values", () => {
            ...
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("union");
            expect(result.members).toHaveLength(2); // string + literal (deduplicated)
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("literal");
            // The literal value will be the last one processed (true)
            expect(result.members[1].value).toBe(true);
        });
        it("should handle union with different primitive types and literals", () => {
            ...
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("float64");
            expect(result.members[2].kind).toBe("literal");
            expect(result.members[2].value).toBe("status");
        });
        it("should demonstrate uniqueUnions deduplication behavior", () => {
            ...
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("union");
            expect(result.members).toHaveLength(3); // deduplicated
            expect(result.members[0].kind).toBe("string");
            expect(result.members[1].kind).toBe("float64");
            expect(result.members[2].kind).toBe("boolean");
        });
    ...
    describe("optional types", () => {
        ...
        it("should handle non_optional schema", () => {
            const schema = valibot.nonOptional(valibot.optional(valibot.string()));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("string");
            expect(result.isOptional).toBe("required");
        });
        ...
        it("should convert optional with complex wrapped type", () => {
            const schema = valibot.optional(valibot.object({
                name: valibot.string(),
                age: valibot.number(),
            }));
            const result = valibotToConvex(schema);
            ...
            expect(result.fields.name.isOptional).toBe("required");
            expect(result.fields.age.kind).toBe("float64");
            expect(result.fields.age.isOptional).toBe("required");
        });
        ...
        it("should convert optional union", () => {
            const schema = valibot.optional(valibot.union([valibot.string(), valibot.number()]));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("union");
            expect(result.isOptional).toBe("optional");
            expect(result.members).toHaveLength(2);
            expect(result.members[0].isOptional).toBe("required");
            expect(result.members[1].isOptional).toBe("required");
        });
    });
    describe("nullable types", () => {
        it("should convert nullable schema", () => {
            ...
            expect(result.kind).toBe("union");
            expect(result.members).toHaveLength(2);
            expect(result.members[0].kind).toBe("null");
            expect(result.members[1].kind).toBe("string");
        });
        ...
        it("should handle non_nullable schema", () => {
            const schema = valibot.nonNullable(valibot.nullable(valibot.string()));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("string");
        });
    ...
    describe("nullish types", () => {
        ...
        it("should handle non_nullish schema", () => {
            const schema = valibot.nonNullish(valibot.nullish(valibot.string()));
            const result = valibotToConvex(schema);
            expect(result.kind).toBe("string");
        });
    ...
    describe("enum and picklist types", () => {
        it("should convert enum schema", () => {
            ...
            const values = result.members.map((m) => m.value);
        ...
        it("should convert status enum with specific values", () => {
            ...
            const member1 = result.members.find((m) => m.value === "draft");
            const member2 = result.members.find((m) => m.value === "published");
            const member3 = result.members.find((m) => m.value === "archived");
    ...
    describe("variant types", () => {
        it("should convert variant schema", () => {
            ...
            expect(result.kind).toBe("union");
            expect(result.members).toHaveLength(2);
            expect(result.members.every((m) => m.kind === "object")).toBe(true);
        });
    ...
    describe("unsupported types", () => {
        it("should throw error for blob", () => {
            const schema = { type: "blob" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Blob is not supported");
        });
        it("should throw error for file", () => {
            const schema = { type: "file" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("File is not supported");
        });
        it("should throw error for date", () => {
            const schema = valibot.date();
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Date can be saved in Convex");
        });
        it("should throw error for function", () => {
            const schema = { type: "function" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Function is not supported");
        });
        it("should throw error for intersect", () => {
            const schema = { type: "intersect" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Not supported : use object merging");
        });
        it("should throw error for lazy", () => {
            const schema = { type: "lazy" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Lazy schemas are not supported");
        });
        it("should throw error for never", () => {
            const schema = { type: "never" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Never is not supported");
        });
        it("should throw error for promise", () => {
            const schema = { type: "promise" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Promise is not supported");
        });
        it("should throw error for void", () => {
            const schema = { type: "void" };
            expect(() => valibotToConvex(schema)).toThrow(ConvexError);
            expect(() => valibotToConvex(schema)).toThrow("Void schema is not supported");
        });
    });
    ...
    describe("complex nested schemas", () => {
        it("should handle deeply nested object with arrays and unions", () => {
            const schema = valibot.object({
                users: valibot.array(valibot.object({
                    id: valibot.number(),
                    name: valibot.string(),
                    email: valibot.optional(valibot.string()),
                    role: valibot.union([
                    ...
                    metadata: valibot.nullable(valibot.record(valibot.string(), valibot.union([
                        valibot.string(),
                        valibot.number(),
                        valibot.boolean(),
                    ]))),
            ...
            expect(result.fields.users.kind).toBe("array");
            const userElement = result.fields.users.element;
            ...
            expect(result.fields.settings.kind).toBe("object");
            expect(result.fields.settings.fields.theme.kind).toBe("string"); // optional unwrapped
            expect(result.fields.settings.fields.notifications.kind).toBe("boolean");
        });
        it("should validate complete e-commerce schema structure", () => {
            const schema = valibot.object({
                products: valibot.array(valibot.object({
                    id: valibot.string(),
                    name: valibot.string(),
                ...
                orders: valibot.record(valibot.string(), valibot.object({
                    status: valibot.union([
                        valibot.literal("pending"),
                        valibot.literal("shipped"),
                        valibot.literal("delivered"),
                    ]),
                    items: valibot.array(valibot.object({
            ...
            expect(result.fields.products.kind).toBe("array");
            const productElement = result.fields.products.element;
            ...
            expect(productElement.fields.categories.element.kind).toBe("string");
            expect(productElement.fields.inventory.kind).toBe("object");
            expect(productElement.fields.inventory.fields.warehouse.kind).toBe("literal");
            // Orders record validation
            expect(result.fields.orders.kind).toBe("record");
            expect(result.fields.orders.key.kind).toBe("string");
            const orderValue = result.fields.orders.value;
        ...
        it("should validate blog schema with convexIds", () => {
            const schema = valibot.object({
                posts: valibot.array(valibot.object({
                    _id: convexId("posts"),
                    ...
                    metadata: valibot.object({
                        ...
                        relatedPosts: valibot.optional(valibot.array(convexId("posts"))),
                    }),
                })),
                users: valibot.record(valibot.string(), valibot.object({
                    _id: convexId("users"),
                    name: valibot.string(),
                    email: valibot.string(),
            ...
            const postsArray = result.fields.posts;
            ...
            expect(metadata.fields.relatedPosts.element.tableName).toBe("posts");
            // Users record validation
            const usersRecord = result.fields.users;
            ...
            expect(userValue.fields.followedBy.members[0].element.kind).toBe("id");
            expect(userValue.fields.followedBy.members[0].element.tableName).toBe("users");
